...
public class Answer {   
    public static int answer(int food, int[][] grid) { 
        ...
        // the food cost for each possible route
	// use HashSet to eliminate duplicates
        List<List<HashSet<Integer>>> cost = new ArrayList<>();
        // compute the cost
        costCompute(food, grid, cost);
        // find the closest value to food in the bottom right room
        int ans = food;
        for (Integer i : cost.get(cost.size() - 1)).get(cost.size() - 1)) {
            int remain = food - i;
            if (remain >= 0) {
                ans = Math.min(ans, remain);
            }
        }
        return ans == food ? -1 : ans;
    } 
    
    private static void costCompute(int food, int[][] grid, List<List<HashSet<Integer>>> cost) {
        int n = grid.length;
        int m = grid[0].length;
	    ...
	    cost.add(list);
            for (int j = 0; j < m; j++) {
                ...
                if (i > 0) {
                    HashSet<Integer> above = cost.get(i - 1).get(j);
                    for (Integer k : above) {
                        tmp.add(k + grid[i][j]);
                    }
                }
                if (j > 0) {
                    HashSet<Integer> left = cost.get(i).get(j - 1);
                    for (Integer k : left) {
                        tmp.add(k + grid[i][j]);
                    }
                }
